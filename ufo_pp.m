...
C = [1 0]; %%%Angulo
D = 0;
E = abs(log(10/100))/sqrt(pi^2 + (log(10/100)^2));
Wn = 4 / (E*3);
wd = Wn*sqrt(1-E^2);
p1 = -E*Wn+Wn*sqrt(E^2-1);
p2 = -E*Wn-Wn*sqrt(E^2-1);
K = place(A, B, [p1 p2]);
plantUC = ss(A-B*K, B, C, 0);

Kp = 1/-(C*inv(A-B*K)*B);
Error = 1 + (C*inv(A-B*K)*B);
plantSEUC = ss(A-B*K, B*Kp, C, 0); 
% Run response to initial condition
t = 0:0.005:15;
[y,t,x] = initial(plantSEUC, x0, t);
toc
