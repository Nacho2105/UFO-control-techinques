...
% Control Law
fis = mamfis('Name',"UFO");
fis = addInput(fis,[0 pi],'Name',"angulo");
fis = addMF(fis,"angulo","trapmf",[0.001 (pi/4)-0.1 (pi/4)+0.1 pi/2],'Name',"De 0 a 90");
fis = addMF(fis,"angulo","trapmf",[pi/2 (3*pi/4)-0.1 (3*pi/4)+0.1 pi],'Name',"De 90 a 180");
fis = addMF(fis,"angulo","trapmf",[pi (5*pi/4)-0.1 (5*pi/4)+0.1 3*pi/2],'Name',"De 180 a 270");
fis = addMF(fis,"angulo","trapmf",[3*pi/4 (7*pi/4)-0.1 (7*pi/4) 2*pi],'Name',"De 270 a 360");
fis = addInput(fis,[-6 6],'Name',"velocidad");
fis = addMF(fis,"velocidad","trapmf",[-6 -3+0.1 -3-0.1 0],'Name',"rapido");
fis = addMF(fis,"velocidad","trapmf",[0 3+0.1 3-0.1 6],'Name',"medio");
%plotmf(fis,"input",1)
fis = addOutput(fis,[0 0.001],'Name',"SalidaA");
fis = addMF(fis,"SalidaA","gaussmf",[0.001  0.001],'Name',"De 0 a 90");
...
H = patch(ufo_rot(:, 1), ufo_rot(:, 2) + yoffset, 'y'); %Crear la animacion
...
text(-2.5, 3, 'Tiempo:         sec', 'Color', [1 1 1], 'FontSize', 24,...
    'HorizontalAlignment', 'left');
text(-2.5, 2.7, 'Gasolina:          units', 'Color', [1 1 1], 'FontSize', 24,...
    'HorizontalAlignment', 'left');

TME = text(-0.3, 3, num2str(current_time, '%.1f'), 'Color', [0 1 0], ...
'FontSize', 24, 'HorizontalAlignment', 'right');
FUL = text(-0.3, 2.7, num2str(fuel, '%.0f'), 'Color', [1 0 0], ...
'FontSize', 24, 'HorizontalAlignment', 'right');

accel_true = diff(x(:, 2))/(t(2)-t(1)); %Derivada de la velocidad
...
for i = 1:5:length(x)-1
    ...
    % Simulation ending condition light cow
    ...
    tiempo_trasncurrido = t(i);
    combustible = fuel*30;
    funcion = tiempo_trasncurrido + combustible;
   % x = fmincon(funcion,0,tiempo_trasncurrido,combustible)
